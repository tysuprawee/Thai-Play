...
export default function Navbar() {
    ...
    useEffect(() => {
        ...
        const channel = supabase
            ...
            // 1.5 New Order Notification (For Sellers)
            .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'orders', filter: `seller_id=eq.${user.id}` }, async (payload) => {
                console.log('Navbar: New Order Received', payload)

                const newOrder = payload.new as any
                // Fetch listing details for the toast
                ...
                const audio = new Audio('/sounds/chaching.mp3') // User needs to add this file or fallback
                ...
                toast.success(`🎉 ขายสินค้าได้แล้ว!`, {
                    ...
                    action: {
                        label: 'ดูออเดอร์',
                        onClick: () => router.push(`/orders/${newOrder.id}`)
                    }
                })

                // Refresh data to show new notification if generated by trigger
                fetchData()
            })
                        ...
                        <>
                            ...
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent className="w-56 bg-card border-white/10 text-white" align="end">
                                    ...
                                    <DropdownMenuItem asChild className="focus:bg-white/10"><Link href="/dashboard">{t.navbar.dashboard}</Link></DropdownMenuItem>
